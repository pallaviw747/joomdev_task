...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == 'POST') {
        ...
        if($email_error === '' && $password_error === '') {
            ...
            $sql = "SELECT * FROM users WHERE email_id = '" . $email . "' AND status = 1 AND role = 'user'";
            ...
            if(!empty($result)) {
                ...
                if (strlen($password) == 32) {
                    if (md5($input_password) === $password) {
                        ...
                        // check when was the last password changed
                        $last_password_change = $result['last_password_change'];
                        $last_login           = $result['last_login'];

                        if($last_login == NULL && $last_password_change == NULL) {
                            // redirect user to update password for the first time
                            header('Location:update_user_password.php');
                        } else if($last_login != NULL && $last_password_change != NULL) {
                            // check last password changed date and redirect to update password if password updated 30 days ago
                            $last_password_change_date = date('Y-m-d', strtotime($last_password_change));
                            $current_date = date('Y-m-d');
                            $days_diff = abs(strtotime($current_date) - strtotime($last_password_change_date));
                            $days_diff = floor($days_diff / (60 * 60 * 24));
                            if($days_diff >= 30) {
                                header('Location:update_user_password.php');
                            } else {
